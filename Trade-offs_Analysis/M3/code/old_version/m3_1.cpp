...
//   vector<Mat> frame_seq;
  ...
  for( i = 0; i < NUM_THREADS; i++ ) {  
      if(NUM_THREADS != 1) tdata[i].frame = imageSections[i];
      else tdata[i].frame = frame;        
  ...
  ofstream file;  
  string outf = "M3output/m3_num_"+to_string(NUM_THREADS);                      // for convience and making graphs easily, the output is saved in a csv file (output.csv)
  file.open(outf);
  //file << "output" << endl;
  ...
  while(1){
    ...
    tdata[i].id = i;
    if(NUM_THREADS != 1) tdata[i].frame = split1[i];
    else tdata[i].frame = fr;
    rc = pthread_create(&threads[i], &attr, execute, (void *) &tdata[i]);
    if (rc) {
      cout << "Error:unable to create thread," << rc << endl;
      exit(-1);
